...
filetosave=directory_to_save+"IASMOS_wake_model2_roughness_"+str(zo)+"_2022.xlsx"
filetowrite = pd.ExcelWriter(filetosave, engine='xlsxwriter')##excel file to save processed data

#power output considering wake losses
...
WDC2=0.5*TI4#wake decay coefficient version 2 , this version seems to overestimate , while version 1 seems to underestimate compared to the constant value used in WaSP for onshore WFs, used under neutral stability conditions
WDC3=0.075#wake decay coefficient version 3
WDC=WDC2## switch between WDC3, WDC2 and WDC1, if it is WDC1 it is 3 values, each for each different hub height config.
    ...
    for bb in range(Density_wtg.shape[1]):
           ...
           for ee in range(Density_wtg.shape[1]):
               if ee!=bb:
                  ...
                  cone_of_infl=math.acos(comp1/math.sqrt((comp2**2)+(comp3**2)))## angle corresponding to the cone of wake influence of upwind to downwind turbine in radians
                  cone_of_infl=cone_of_infl*(180/math.pi) ## the aforementioned angle in degrees.
                  ...
                  ## if turbine bb is in the cone of wake influence of upwind turbine ee ( cone angle lower than or equal to angle threshold ,keep downwind distance at particular wind direction and also derive radial distance from the wake center
                  if cone_of_infl>0 and cone_of_infl<=angle_thres:
                     D_down=d_down
                     rd=D_down*math.sin(math.radians(cone_of_infl))## radial distance
                  ...
                  Ashadow=0
                  overlap_f=0
                  print('WTG ' + str(bb + 1) + ' is the same as WTG ' + str(ee + 1))
               ...
               for ff in range(Density_wtg.shape[2]):
                   ...
                   del windextr
                   if D_down>0:
                      w_red_f1=(1-math.sqrt(1-thrustextr))*((RR/(RR+(WDC*D_down)))**2) ## wind reduction factor of target turbine from upwind turbine , due to wake effect, as derived from modified Jensen model for the axial distance.
                      w_red_f2=(5.16/math.sqrt(2*math.pi))*math.exp((-(rd*2.58)**2)/(2*(Rwake**2))) ## wind reduction factor of target turbine as derived from Gaussian model for radial distance wake influence
                      w_red_f=w_red_f1*w_red_f2*overlap_f
                   else:
                       w_red_f=0
